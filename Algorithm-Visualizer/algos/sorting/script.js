...
console.log("status before " + isSecondOption);
...
let arrayForUserInput;
let arrSum;

let arrayElementCounter;
function userInput() {
  arrayForUserInput=[];
  arrSum=0;

  isSecondOption =true;
  let user = document.querySelector("#userArrInput")
  let userArrayInput = user.value;
  let n = userArrayInput.split(' ');
  let  i= 0
  let sumForInput =0 ;
  // if(n.length = 0)alert("Please input some value");
for (i;  i< n.length; i++) {
    
    arrayForUserInput[i] = parseInt(n[i]);
    sumForInput = sumForInput + parseInt(n[i]);
  // console.log(arrayForUserInput[i],sumForInput+"\n");
  // console.log("status after " + isSecondOption);
  
}
arrayElementCounter = i;
arrSum = sumForInput;
height=container.offsetHeight;
generateRandomArray();
}

function getHvalue(i){
  let value = (arrayForUserInput[i]/arrSum)*100;
  let newHpercent=height/100;
  return (newHpercent*(value*9));
}
...
function generateRandomArray() {
  ...
  n = (isSecondOption) ? arrayElementCounter : barSlider.value;
  lineWidth = width / n - 1.2;
  container.innerHTML = '';
  for (let i = 0; i < n; i++) {
    heights[i] = (isSecondOption)?  parseInt(getHvalue(i)) : parseInt(getRandomValue(1, height));
    bars.push(document.createElement('div'));
    bars[i].style.width = `${lineWidth-3}px`;
    ...
    if (n <= 60) {
      ...
      barValues[i].innerHTML = (isSecondOption)? arrayForUserInput[i]:heights[i];
  ...
  for (let i = 0; i < n - 1; i++) {
    ...
    for (let j = i + 1; j < n; j++) {
      ...
      if (!isPaused) {
        ...
        draw([i, j, minIndex], ['yellow', 'red', 'green']);
      } else {
        j--;
      }
      await sleep(delay);
      ...
      while (i < n1 || j < n2) {
        ...
        if (!isPaused) {
          if (j == n2 || (i < n1 && L[i] <= R[j])) {
            ...
            draw([k, ...barsIndices], ['red', ...barsColors]);
            j++;
          }
          k++;
        }
...
document.getElementById('sortButton').addEventListener('click', () => {
  ...
  console.log("status end " + isSecondOption);
...
document.getElementById('stopButton').addEventListener('click', () => {
  isStopped = true;
  isPaused = false;
  document.getElementById('pauseButton').innerHTML = '<img src="../../images/pause.png" alt="Pause" srcset="" class="play">';
...
document.getElementById('pauseButton').addEventListener('click', () => {
  ...
  if (!isStopped) {
    ...
    if (isPaused) {
      ...
      document.getElementById('pauseButton').innerHTML = '<img src="../../images/play.png" alt="Resume" srcset="" class="play">';
      isPaused = true;
    }
  }
});
